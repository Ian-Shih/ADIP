...
void hw2_b() {
	...
	imwrite("rect_dft_by_cv.png", test);
	imshow("rect_idft_by_cv", cv1);
	imwrite("rect_idft_by_cv.png", cv1);
	waitKey(0);
	destroyAllWindows();

	imshow("circle_dft_by_cv", test1);
	imwrite("circle_dft_by_cv.png", test1);
	imshow("circle_idft_by_cv", cv2);
	imwrite("circle_idft_by_cv.png", cv2);
	waitKey(0);
	destroyAllWindows();

	imshow("square_dft_by_cv", test2);
	imwrite("square_dft_by_cv.png", test2);
	imshow("square_idft_by_cv", cv3);
	imwrite("square_idft_by_cv.png", cv3);
	waitKey(0);
	destroyAllWindows();

	imshow("diamond_dft_by_cv", test3);
	imwrite("diamond_dft_by_cv.png", test3);
	imshow("diamond_idft_by_cv", cv4);
	imwrite("diamond_idft_by_cv.png", cv4);
	waitKey(0);
	destroyAllWindows();
}
int main() {
